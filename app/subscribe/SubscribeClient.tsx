...
export function SubscribeClient({ subscription: initialSubscription, userId }: SubscribeClientProps) {
  ...
  async function handlePayment() {
    ...
    const PortOne = typeof window !== "undefined" ? (window as any).PortOne : null;
    if (!PortOne) {
    ...
    try {
      ...
      if (confirmData.ok !== true) {
        ...
        const is401 =
          confirmRes.status === 401 || (reason && String(reason).includes("API Secret"));
        alert(
          is401
  ...
  function getBonusRate(amt: number) {
    if (amt >= 3_000_000) return 0.3;
    if (amt >= 1_000_000) return 0.2;
    if (amt >= 300_000) return 0.1;
    return 0;
  }
  ...
  return (
    <div className="space-y-6">
      <Card>
        ...
        <p className="mt-1 text-base font-medium text-zinc-900">{subscriptionStatusText}</p>
      </Card>

      <Card>
        <p className="text-sm font-medium text-zinc-500">결제금액</p>
        <p className="mt-1 text-2xl font-bold text-zinc-900">{formatKRW(selectedAmount)}</p>
      </Card>

      <div className="flex items-center justify-between gap-3 rounded-xl border bg-white p-4">
        <div className="flex min-h-[56px] min-w-[110px] flex-col items-center justify-center rounded-lg border bg-gray-50">
          <div className="text-xs text-gray-500">보너스율</div>
          <div className="text-lg font-bold text-blue-600">{bonusRate * 100}%</div>
        </div>

        <div className="flex-1">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-500">보너스금액</div>
            <div className="text-sm font-semibold text-gray-900">{formatKRW(bonusAmount)}</div>
          </div>

          <div className="mt-2 flex items-center justify-between">
            <div className="text-sm text-gray-500">총충전금액</div>
            <div className="text-sm font-semibold text-gray-900">{formatKRW(totalCredit)}</div>
          </div>
        </div>
      </div>
