  ...
  async function handleChargePayment() {
    ...
    try {
      ...
      if (!prepareRes.ok) {
        ...
        console.error("[충전] prepare 실패:", prepareRes.status, prepareData);
        alert(errMsg);
        setIsPaying(false);
        return;
      }
      ...
      if (!merchant_uid) {
        console.error("[충전] prepare 응답 오류:", prepareData);
      ...
      if (!buyerPhone) {
        buyerPhone = window.prompt("결제용 휴대폰번호를 입력해 주세요 (예: 01012345678)")?.trim() ?? "";
      }
      ...
      if (!imp_uid) {
        console.error("[충전] PortOne 응답:", result);
      ...
      for (let attempt = 0; attempt < maxRetries; attempt++) {
        ...
        console.log("[충전] confirm response", confirmRes.status, confirmData);
        ...
        const isInProgress = reason.includes("결제 미완료") ||
      ...
      if (confirmData.ok !== true) {
        const reason = confirmData.reason ?? "";
        let msg: string;
        if (reason.includes("API Secret") || (confirmData.detail as { httpStatus?: number })?.httpStatus === 401) {
          msg = "결제는 완료됐을 수 있는데 서버 확인이 실패(401)해서 적립이 안 됨. API Secret 확인 필요";
